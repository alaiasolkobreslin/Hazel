...
and 'a prog =
  'a * 'a open_stmnt list * 'a alias list * 'a let_defn list * 'a expr_ann
...
and expr_restricted =
  ...
  | RMatchWithWhen of
      (expr_restricted
      * (expr_restricted * expr_restricted option * pattern_restricted) list)
...
let uop_to_sexpr uop =
  let str =
    match uop with Not -> "not" | Neg -> "-" | Ref -> "ref" | Deref -> "!"
  in
  SNode str
...
let rec rexpr_to_sexpr = function
  ...
  | RBinop (bop, e1, e2) ->
      SList [ bop_to_sexpr bop; e1 |> rexpr_to_sexpr; e2 |> rexpr_to_sexpr ]
  | RUnaop (uop, e) -> SList [ uop_to_sexpr uop; e |> rexpr_to_sexpr ]
  | RCons (e1, e2) ->
      SList [ SNode "::"; e1 |> rexpr_to_sexpr; e2 |> rexpr_to_sexpr ]
  | RConstructor (str, e) -> SList [ SNode str; e |> rexpr_to_sexpr ]
  | RRecord lst ->
      SList
        (List.map
           (fun (str, e) -> SList [ SNode str; e |> rexpr_to_sexpr ])
           lst)
...
and expr_to_sexpr = function
  ...
  | Fun (arg, e) ->
      SList
        [
          SNode "lambda"; arg |> snd |> pat_to_sexpr; e |> snd |> expr_to_sexpr;
        ]
  | App (e1, e2) ->
      SList [ e1 |> snd |> expr_to_sexpr; e2 |> snd |> expr_to_sexpr ]
  | Binop (bop, e1, e2) ->
      SList
        [
          bop_to_sexpr bop;
  ...
  | Record lst ->
      SList
        (List.map
           (fun (str, e) -> SList [ SNode str; e |> snd |> expr_to_sexpr ])
           lst)
...
and match_with_when_to_sexpr_help = function
  ...
  | (e, None, p) :: t ->
      [ SNode "|"; pat_to_sexpr (snd p); SNode "->"; e |> snd |> expr_to_sexpr ]
      @ match_with_when_to_sexpr t
...
and rexpr_match_with_when_to_sexpr_help = function
  ...
  | (e, None, p) :: t ->
      [ SNode "|"; rpat_to_sexpr p; SNode "->"; e |> rexpr_to_sexpr ]
      @ rexpr_match_with_when_to_sexpr t
...
and pat_to_sexpr = function
  ...
  | PString s | PVar s -> SNode s
  | PTup lst -> SList (List.map (fun elt -> elt |> snd |> pat_to_sexpr) lst)
  | PSum (str, p) -> SList [ SNode str; p |> snd |> pat_to_sexpr ]
  | PNil -> SNode "[]"
  | PCons (p1, p2) ->
      SList [ p1 |> snd |> pat_to_sexpr; SNode "::"; p2 |> snd |> pat_to_sexpr ]
...
and rpat_to_sexpr = function
  ...
  | RPString s | RPVar s -> SNode s
  | RPTup lst -> SList (List.map (fun elt -> elt |> rpat_to_sexpr) lst)
  | RPSum (str, p) -> SList [ SNode str; p |> rpat_to_sexpr ]
  | RPNil -> SNode "[]"
  | RPCons (p1, p2) ->
      SList [ p1 |> rpat_to_sexpr; SNode "::"; p2 |> rpat_to_sexpr ]

and open_to_sexpr = function _, str -> SList [ SNode "open"; SNode str ]
...
let prog_to_sexpr (prog : 'a prog) =
  ...
  | _, l1, l2, e, ex ->
      SList
        ([
           SList (List.map open_to_sexpr l1); SList (List.map alias_to_sexpr l2);
         ]
