...
module HashSet : HashSet = struct
  ...
  let add (h : 'a t) (x : 'a) = if mem h x then () else Hashtbl.add h x x
  ...
  let values (h : 'a t) : 'a list = Hashtbl.fold (fun x y v -> y :: v) h []
end
...
module LexStream : LexStream = struct
  ...
  let rec inc (s : int list) : int list =
    ...
    | [] -> [ Char.code 'a' ]
    | x :: t ->
        if x < Char.code 'z' then (x + 1) :: t else Char.code 'a' :: inc t
...
module Fresh : Fresh = struct
  ...
  let next (s : t) : string =
    ...
    let rec check n =
      if HashSet.mem avoid n then check (LexStream.next stream) else n
    in
...
let rec remove_duplicates (s : 'a list) : 'a list =
  ...
  | [] -> []
  | x :: t ->
      if List.mem x t then remove_duplicates t else x :: remove_duplicates t

(* remove all elements of list t from list s *)
let remove_all t s = List.fold_left (fun s x -> List.filter (( <> ) x) s) s t
