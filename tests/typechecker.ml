...
let make_typed_expr_test
    (name : string) 
    (expr : parsed expr_ann) 
    (env : (constructor * types) list)
    (expected_output : typed expr_ann) : test = 
  name >:: (fun _ -> 
      assert_equal expected_output (typecheck_expr expr [] env))

let basic_expr_tests = [
  make_typed_expr_test "true is bool"
  ({parsed_pos = default_position; ptype = None}, (Bool true)) default_env
  ({typed_pos = default_position; ttype = TBool}, (Bool true));
  make_typed_expr_test "false is bool"
  ({parsed_pos = default_position; ptype = None}, (Bool false)) default_env
  ...
  default_env ({typed_pos = default_position; ttype = TBool}, 
