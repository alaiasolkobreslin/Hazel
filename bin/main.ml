...
let report_lex_error file_name out_channel pos message =
  ...
  if out_channel = stdout then () else output_string out_channel error_msg;
  prerr_endline
    ("Lexical error beginning at " ^ file_name ^ ":" ^ l_num ^ ":" ^ c_num
   ^ ": " ^ message)
...
let lex_channel write_out in_channel out_channel file_name print_std =
  try
    ...
    let rec step_lexer op =
      ...
      | None -> ()
      | Some tok ->
          if write_out then output_string out_channel tok;
...
let chop_file file_name ext =
  ...
  if Filename.check_suffix file_name ".haze" then
    Some (Filename.chop_suffix file_name ".haze" ^ ext)
  else None
...
let write_out_file func out_dir file_name ext print_std =
  let file_out = (Filename.concat out_dir file_name |> chop_file) ext in
  match file_out with
  | None -> print_endline ("Invalid file extension for file: " ^ file_name)
  | Some file -> (
      (try Filename.dirname file |> mkdir_path
       with _ -> print_endline "Filed to make directory");
      try
        func true (open_in file_name) (open_out file) file_name print_std
        |> ignore
...
let perform_commands commands files out_dir print_std =
  if List.exists (fun c -> c = Lex) commands then
    List.map
      (fun f -> write_out_file lex_channel out_dir f ".lexed" print_std)
      files
    |> ignore
  else if List.exists (fun c -> c = Parse) commands then
    List.map
      (fun f -> write_out_file parse_channel out_dir f ".parsed" print_std)
      files
    |> ignore
  else ()
...
let _ =
  ...
  | { commands; files; out_dir; print_std } ->
      perform_commands commands files out_dir print_std
